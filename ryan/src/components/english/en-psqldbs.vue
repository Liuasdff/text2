a<template>
  <div class="applications" v-show="routers">
    <div class="global-search" :class="{'search':globalsearch ==false}">
      <button class="tianjia" @click="add">十 添加 SQL 数据库</button>
      <span>
        <input class="sousuo" @input="sousuochange" v-model="sousuo" placeholder="搜索 SQL 数据库" />
      </span>
    </div>
  ...
  export default {
    ...
    mounted() {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          //             }
          //           }
          //           that.data[j].dbusers_readwrite_name = []
          //           for(var k = 0;k<that.data[j].dbusers_readwrite.length;k++){
          //             for(var kk = 0;kk<data3.length;kk++){
          //               if(that.data[j].dbusers_readwrite[k] == data3[kk].id){
          //                 that.data[j].dbusers_readwrite_name.push(data3[kk].name)
          //                 break
          //               }
          //             }
          //           }
    ...
    methods: {
      ...
      adm(k) {
        ...
        this.$router.push({path: '/en-psqldbs-adm',query:{}})
