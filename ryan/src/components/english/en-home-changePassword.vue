	...
	export default{
      ...
      xiugai(){
        if(this.password !=""){
          ...
          //   data.push({'id':this.loca[i].id,'password':this.password})
          //   console.log()

          // }
          // if(window.confirm("确定修改密码吗？")){
            // var count1 = 0
            data.push({'username':localStorage.getItem('home-username'),'password':this.password})
            console.log(data)
            $(".xiugai").attr('disabled',true);
            // for(var j = 0;j<data.length;j++){
              $.ajax({
                 // url: URL + "/osuser/pwdch/",
                 headers:token,
