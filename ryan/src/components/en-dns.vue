a<template>
  <div class="applications" v-show="routers">
    <div class="global-search" :class="{'search':globalsearch ==false}">
      <button class="tianjia" @click="add">十 Add DNS Record</button>
      <span>
        <input class="sousuo" @input="sousuochange" v-model="sousuo" placeholder="Search DNS Records" />
      </span>
    </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      var that = this
      $.ajax({
        url: URL + "/dnsrecord/list/?embed=domain",
        headers: token,
        type: "GET",
    ...
    methods: {
      ...
      sousuochange() {
        ...
        this.data1 = this.data.filter(function(k, i) {
          return k.domain.name.indexOf(that.sousuo) != -1
        })
      ...
      update(k) {
        localStorage.setItem('dns-update',JSON.stringify(k))
        this.$router.push({ path: '/en/dns-update'})
