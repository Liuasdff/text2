<template>
  <div>
    ...
    <div class="bodydiv">
      ...
      <div class="div4">
        <i></i><span>Please specify a destination mail user and server for mailbox storage. Mail users must be
          installed before they may be selected here. </span>
      </div>
      <div id="mailnewcl">
        <div class="clonediv" v-for="(k,i) in obj.destinations" :key='i'>
          ...
          <span class="clonedel"><button @click="delone(i)">Delete</button></span>
        </div>
      </div>
      <div class="div5"><button @click="addd"><i>十</i><span>Add Destination Mail User</span></button></div>
      <div class="mdd">Forwards</div>
      <div class="div4">
        <i></i><span>Please specify a list of forward email addresses. Our mail servers will attempt to forward to
          these addresses. For critical messages it is suggested you add a destination mail user as well.</span>
      </div>
      <div id="<mailnewcl2></mailnewcl2>">
        <div class="clonediv2" v-for="(k,i) in cltow" :key='i'>
          <div class="div3 div33">
            <input class="mailnewclone1" v-model="k[0]" placeholder="you" />
            <span class="a">@</span>
            <input class="mailnewclone2" v-model="k[1]" placeholder="example.com" />
          </div>
          <span class="clonedel2"><button @click="deltow(i)">Delete</button></span>
        </div>
      </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      addd() {
        this.onecount += 1
        this.obj.destinations.push({
          'id': this.option2[0].id
        })
        // this.clone.push({
        //   'id': this.onecount,
        // })
