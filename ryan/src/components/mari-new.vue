<template>
  <div class='mariauser-new'>
    <span class='span1'>新Maria数据库</span>
    ...
    <div class='div2'>
      ...
      <div class='div2-2' v-show="css1 == 1">
        <div class='div2-2-1'>Maria DB是MySQL兼容的数据库，具有许多增强功能。它由MySQL的原始开发人员制作，并保证保持开源。</div>
        <div>名称</div>
        <div class='dv2'><input v-model="input1"></div>
        <div class='dv3'>用于请求Maria DB数据库的名称，以及分配用于访问该数据库的用户。</div>
        <div>服务器</div>
        <div class='dv2'>
          <span class="option">
        ...
        <div class='dv3'>您的Maria数据库的字符集。</div>
        <div>读写Maria用户。</div>
        <div class='dv2'>
          <span class="option">
      ...
      <div class = "btn">
        <button class = 'add' @click="add">添加Maria数据库</button>
...
<script>
  ...
  export default {
    ...
    methods: {
      quxiao(){
        this.$router.push({path:'/mariadbs'})
      },
      add() {
        var that = this
