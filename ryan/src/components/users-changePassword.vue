    ...
    <form>
      <div class="text">
        <div class="div2">
          ...
          <div class="names" v-for="(k,i) in loca"> 选定的系统用户 {{ loca[i].mykey.name }}@{{ loca[i].mykey.server.hostname }}</div>
          <div class="div2-2">
            <div class='divv2-2'>
              密码在8到64个字符之间。
            </div>
          ...
          <div><input @input="input1" maxlength="64" v-model="newpass" type="password" autocomplete='false' placeholder="新密码"></div>
          <div class="querengpass">确认密码 &nbsp; <span :style="{'color':color2}">{{ text2 }}</span></div>
          <div><input @input="input2" autocomplete='false' maxlength="64" v-model="querenpass" type="password" placeholder="确认密码"></div>
