a<template>
  <div class="applications" v-show="routers">
    <div class="global-search" :class="{'search':globalsearch ==false}">
      <button class="tianjia" @click="add">十 Add Maria Database</button>
      <span>
        <input class="sousuo" @input="sousuochange" v-model="sousuo" placeholder="Search Maria Database" />
      </span>
    </div>
    <span>Maria Database</span>
    <div class="div3">
...
<script>
  ...
  export default {
    ...
    mounted() {
      var that = this
      $.ajax({
        url: URL + "/mariadb/list/?embed=mariauser,server,dbusers_readwrite,dbusers_readonly",
        headers: token,
        type: "GET",
  ...
  methods: {
      ...
      update(k) {
        localStorage.setItem('mariadbs-update', JSON.stringify(k))
        this.$router.push({
          path: '/en/mariadbs-update',
          query: {}
        })
        // 使用name就要用params,使用path就要用query传递参数
        // this.$router.push({name:'/mailusers-update',params: { userId: 123 }})
