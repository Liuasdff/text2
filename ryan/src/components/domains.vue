<template>
  <div class="applications" v-show='routers'>
    <div class="global-search" :class="{'search':globalsearch ==false}">
      <button class="tianjia" @click="add">十 添加域名</button>
      <span>
        <input class="sousuo" @input="sousuochange" v-model="sousuo" placeholder="搜索域" />
      </span>
    </div>
    <span>域</span>
    <div class="div3">
    ...
    <div  v-show='iff'>
      ...
      <div class = 'text'>
        <div class = 'text1'>
          ...
          <div class = 'odd'>DKIM 记录 {{ upname }}</div>
          <div class = 'even texts1'></div>
          <div><button @click="queding" class = "queding">确定</button></div>
        </div>
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      update(name, id) {
        var text = '暂时为空'
        myalert(text)
        // this.$router.push({path:'/mail-update',query: { id: id,name:name }})
