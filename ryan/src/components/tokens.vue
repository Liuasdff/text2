a<template>
  <div class="applications" v-show="routers">
    <div class="global-search" :class="{'search':globalsearch ==false}">
      <button class="tianjia add" @click="add">十 添加 API 令牌</button>
      <span>
        <input class="sousuo" @input="sousuochange" v-model="sousuo" placeholder="搜索 API 令牌" />
      </span>
    </div>
    <span>令牌</span>
    <div class="div3">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      add() {
        this.$router.push({path:'/tokens-new'})
      },
      update(k) {
