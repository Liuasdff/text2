<template>
  <div class="applications" v-show='routers'>
    <div class="global-search" :class="{'search':globalsearch ==false}">
      <button class="tianjia" @click="add">十 Add Mall User</button>
      <span>
        <input class="sousuo" @input="sousuochange" v-model="sousuo" placeholder="Search Mall Users" />
      </span>
    </div>
...
<script>
  ...
  import changePassword from "./en-mailusers-changepass.vue"
  export default {
    data() {
      return {
        routers: false,
    ...
    methods: {
      ...
      add() {
        this.$router.push({
          path: '/en/mailusers-new'
        })
      },
      update(obj) {
        localStorage.setItem('mai-update',JSON.stringify(obj))
        this.$router.push({path: '/en/mailusers-update'})
