<template>
  <div class='applications' v-show="routers">
    ...
    <div class='div1'> Select users to read and write to your database</div>
    <div class="div3">
      <span>You selected {{ checkcount }} users &nbsp;</span>
      <button class="del" @click="del">Save</button>
...
<script>
  ...
  export default {
    ...
    mounted() {
      var loca = JSON.parse(localStorage.getItem('psqldbs-update'))
      // console.log(loca)
    ...
    methods: {
      ...
      appcheckth() {
        ...
        if (checkth) {
          ...
          this.dellist = [{
            'id': this.id,
            'dbusers_readwrite': dbusers_readwrite
          }]
      ...
      del() {
        ...
        if (dellist.length < 1) {
          ...
          var aa = "Are you sure to Add " + that.checkcount + ' items？'
          confirm2('Add psqldbs Read-Write User', aa, fn)
        }
      },
