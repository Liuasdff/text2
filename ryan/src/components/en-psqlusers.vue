a<template>
  <div class="applications" v-show="routers">
    <div class="global-search" :class="{'search':globalsearch ==false}">
      <button class="tianjia" @click="add">十 Add Postgres User</button>
      <span>
        <input class="sousuo" @input="sousuochange" v-model="sousuo" placeholder="Search Postgres Users" />
      </span>
    </div>
    <span>Postgres Users</span>
    <div class="div3">
      <span>You selected {{ checkcount }} records &nbsp;</span>
      <button class="del" @click="del">Delete Selected</button>
      <button class="change" @click="changePassword">Change Server Passwords</button>
    </div>
...
<script>
  ...
  import changePassword from "./en-psqlusers-changepass.vue"
  export default {
    data() {
      return {
        globalsearch: search,
