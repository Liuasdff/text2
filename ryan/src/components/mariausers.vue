a<template>
  <div class="applications" v-show="routers">
    <div class="global-search" :class="{'search':globalsearch ==false}">
      <button class="tianjia" @click="add">十 添加Maria用户</button>
      <span>
        <input class="sousuo" @input="sousuochange" v-model="sousuo" placeholder="搜索Maria用户" />
      </span>
    </div>
    <span>Maria用户</span>
    <div class="div3">
...
<script>
  ...
  import changePassword from "./mariausers-changepass.vue"
  export default {
    data() {
      return {
        globalsearch: search,
