<template>
  <div class='mariauser-new'>
    <span class='span1'>新Maria用户</span>
    ...
    <div class='div2'>
      ...
      <!-- <div>字符集</div>
        ...
        <div class='flex1'>
          <span></span>
          <span>允许外部访问此数据库用户</span>
        </div>

      </div>

      <div class = "btn">
        <button class = 'add' @click="add">添加Maria用户</button>
...
<script>
  ...
  export default {
    ...
    methods: {
      quxiao(){
        this.$router.push({path:'/mariausers'})
      },
      add() {
         var that =this
         ...
         $.ajax({
            ...
            contentType: "application/json",
              ...
              that.$router.push({path:'/mariausers'})
            },
